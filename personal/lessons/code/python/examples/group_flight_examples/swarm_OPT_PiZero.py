...
# IP-адреса дронов
pioneer_ip = ["192.168.137.62", "192.168.137.245"]

# Словари для хранения объектов
pioneer = {}        # объекты Pioneer
...
def wait_connection():
    while True:
        ret = True
        for ip in pioneer_ip:
            if not pioneer[ip].connected():
                ret = False
        if ret:
            return
        time.sleep(0.5)
...
def go_to_start_point(drone_ip):
    ...
    pioneer[drone_ip].go_to_local_point(x=point[0], y=point[1], z=point[2], yaw=0)
    while not pioneer[drone_ip].point_reached():
        pass
...
def run_mission_flight(drone_ip):
    while True:
        ...
        while not pioneer[drone_ip].point_reached():
            pass
        point_reached[drone_ip] = True  # После прилёта в точку отмечаемся
    pioneer[drone_ip].land()  # После окончания миссии выполняется посадка
...
def program():
    for ip in pioneer_ip:  # Создание отдельных потоков выполнения для каждого дрона
        print("Thread " + ip)
    ...
    for coord in coords:
        for ip in pioneer_ip:  # Установка следующей точки для каждого коптера
            tasks[ip] = coord
...
if __name__ == '__main__':
    ...
    for ip in pioneer_ip:
        pioneer[ip] = Pioneer(ip=ip, mavlink_port=5656, name=ip, logger=False)
