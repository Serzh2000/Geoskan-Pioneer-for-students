...
function getPythonCategory(key: string): { category: string; colour: string } {
    if (key.startsWith('Pioneer.')) {
        ...
        if (/led_control/.test(key)) {
            return { category: 'Индикация Python', colour: '#22c55e' };
        }
        if (/send_rc_channels/.test(key)) {
            return { category: 'Управление Python', colour: '#f59e0b' };
        }
...
function buildCatalog(language: ScriptLanguage): ApiCatalogEntry[] {
    const docs = language === 'lua'
        ? Object.entries(apiDocs)
        : Object.entries(pythonApiDocs);
...
function groupCatalogByCategory(entries: ApiCatalogEntry[]): Map<string, ApiCatalogEntry[]> {
...
function renderStandardCategories(language: ScriptLanguage): string {
    ...
    const extraRawBlock = language === 'lua' ? LUA_RAW_CODE_BLOCK : PY_RAW_CODE_BLOCK;
...
export function ensureEditorBlocklyDefinitions(): void {
    ...
    defineRawCodeBlock('lua', LUA_RAW_CODE_BLOCK, 'Lua: вставка кода');
    defineRawCodeBlock('python', PY_RAW_CODE_BLOCK, 'Python: вставка кода');
