...
\end{itemize}
\textbf{Примеры:}
\begin{codefile}{lua/examples/chapter_05/leds_events_landed.lua}
\end{codefile}
\textbf{Пояснение.} В этом листинге функция \verb|callback(event)| отслеживает событие \verb|Ev.COPTER_LANDED| и при посадке выключает все светодиоды, проходя по индексам линейки. Основной принцип — простая реакция на одно конкретное событие автопілота.
\begin{codefile}{lua/examples/chapter_05/leds_events_low_voltage.lua}
\end{codefile}
